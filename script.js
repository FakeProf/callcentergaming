...
document.addEventListener('DOMContentLoaded', () => {
    // Initialisiere Rangliste
    initializeRangliste();
    
    // Initialisiere Anmeldungsbereich
    const anmeldungContainer = document.getElementById('anmeldung-container');
    if (anmeldungContainer) {
        renderAnmeldungBereich();
    }
});
...
async function renderSpieltagListe(wochenId) {
    ...
    try {
        ...
        spieltagContainer.innerHTML = '<p class="error-message">Fehler beim Laden der Anmeldungen</p>';
    }
}
...
async function toggleAnmeldung(spielName) {
    ...
    try {
        const anmeldungen = await fetch(`http://localhost:3000/api/registrations/${spielName}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                ...
                action: isUserRegistered(spielName, session.username) ? 'unregister' : 'register'
            })
        });

        const response = await anmeldungen.json();
        if (response.success) {
            // Aktualisiere die lokale Anzeige
            const activeTab = document.querySelector('.tab-button.active');
            if (activeTab) {
                renderSpieltagListe(activeTab.dataset.woche);
            }
        }
    } catch (error) {
...
async function isUserRegistered(spielName, username) {
    try {
        const response = await fetch('http://localhost:3000/api/registrations');
        const registrations = await response.json();
        return registrations[spielName] && registrations[spielName].includes(username);
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialisiere Anmeldungssektion
...
// Netlify Identity Funktionen
function handleNetlifyLogin(user) {
    const sessionData = {
        username: user.email.split('@')[0], // Verwende den Teil vor dem @ als Benutzernamen
        loginTime: new Date().toISOString(),
        isActive: true,
        email: user.email
    };
    ...
    // UI aktualisieren
    document.getElementById('user-info').textContent = `Angemeldet als: ${sessionData.username}`;
    document.getElementById('login-button').style.display = 'none';
    document.getElementById('logout-button').style.display = 'block';
    
    // Anmeldungsbereich aktualisieren
    updateAnmeldungSection();
    aktualisiereSpielplan();
}
...
// Login/Logout Funktionen aktualisieren
function logout() {
...
// Anmeldungsfunktionen aktualisieren
async function toggleAnmeldung(spielName) {
    const session = JSON.parse(localStorage.getItem('currentSession'));
    if (!session || !session.isActive) {
    ...
    try {
        ...
        const response = await fetch('/.netlify/functions/toggle-registration', {
            ...
            body: JSON.stringify({
                game: spielName,
                username: session.username,
                action: await isUserRegistered(spielName, session.username) ? 'unregister' : 'register'
            })
        });
